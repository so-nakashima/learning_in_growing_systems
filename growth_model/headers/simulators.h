...
public:
    Environments_Sequence(int card, const std::vector<int> &seq)
    {
        m_sequence = seq;
        m_cardinality = card;
    };
...
protected:
public:
    //can be used as default constructor Cell_Learn()
...
protected:
    ...
    std::vector<Cell_Learn> before_selection;
    int maximum_population_size;
    std::mt19937_64 mt;
...
public:
    Cells_Learn(
        ...
        const std::function<void(int, int, int, std::vector<std::vector<double>> &, std::vector<std::vector<double>> &, std::vector<double> &, std::vector<double> &, std::mt19937_64 &)> &rule = [](int _, int __, int _p, std::vector<std::vector<double>> &_a, std::vector<std::vector<double>> &_b, std::vector<double> &_c, std::vector<double> &_d, std::mt19937_64 &_e) {});
    ~Cells_Learn(){};
    ...
    virtual int cardinality() const override { return m_type_cardinality; };
...
private:
    ...
    std::vector<Cell_Learn> next_population; //used to compute the next spine cell

    std::ofstream *out_spine = nullptr;

public:
    //the first element in initial_pop becomes the spine cell. If initial_pop is empty, then the spine cell is set to be default.
    Cells_Learn_Common(
        int type_no = 0,
        int max_pop_size = 1,
        const std::vector<Cell_Learn> &initial_pop = {},
